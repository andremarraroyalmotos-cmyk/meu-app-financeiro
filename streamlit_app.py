...
# --- ABA 1: DASHBOARD (FILTRO BRASILEIRO) ---
if aba == "📊 Dashboard":
    st.markdown("<h1>📊 Painel de Controle</h1>", unsafe_allow_html=True)
    if not df_raw.empty:
        with st.container():
            c_f1, c_f2 = st.columns(2)
            # Formato brasileiro no date_input (DD/MM/AAAA)
            data_inicio = c_f1.date_input("Início", df_raw['data'].min(), format="DD/MM/YYYY")
        ...
        df_view = df_filtrado[['data', 'descricao', 'categoria', 'tipo', 'valor']].sort_values('data', ascending=False).copy()
        # Formata a coluna de data para o padrão brasileiro na visualização da tabela
...
# --- ABA 3: GERENCIAR ---
    ...
    with t_edit:
        if not df_raw.empty:
            # Formatação BR para a lista de seleção
            df_raw['display'] = pd.to_datetime(df_raw['data']).dt.strftime('%d/%m/%Y') + " - " + df_raw['descricao'] + " (R$ " + df_raw['valor'].astype(str) + ")"
            ...
            with st.form("edit_form"):
                c1, c2 = st.columns(2)
                novo_dt = c1.date_input("Data", item['data'], format="DD/MM/YYYY")
                ...
                if col_btn1.form_submit_button("💾 ATUALIZAR"):
                    conn.client.table("lancamentos").update({"data": str(novo_dt), "descricao": novo_ds, "valor": novo_vl, "tipo": novo_tp, "categoria": novo_ct}).eq("id", escolha).execute()
                    st.cache_data.clear()
                    st.success("Alterado!")
                    time.sleep(1)
                    st.rerun()
